...
def hand_value(hand):
    ...
    numAces = sum(1 for card in hand if 'Ace' in card)  # Count the number of Aces in the hand
    while value > 21 and numAces:  # Adjust the value if there are Aces and the total value exceeds 21
        value -= 10
        numAces -= 1
    return value
...
def play_game():
    ...
    while chips > 0:  # Continue playing as long as the player has chips
        ...
        playerHand = [deck.pop(), deck.pop()]  # Deal two cards to the player
        dealerHand = [deck.pop(), deck.pop()]  # Deal two cards to the dealer

        print(f"Your hand is a {' and a '.join(playerHand)} (with a total value of {hand_value(playerHand)})")
        print(f"Dealer's show card: {dealerHand[0]}")

        # Player's turn
        while hand_value(playerHand) < 21:  # Allow the player to continue playing until they bust or stand
            action = get_valid_action()  # Prompt the player for action (hit or stand)
            if action == 'h':
                playerHand.append(deck.pop())  # Deal an additional card to the player
                print(f"Your hand is a {' and a '.join(playerHand)} (with a total value of {hand_value(playerHand)})")
            elif action == 's':
        ...
        # Dealer's turn
        while hand_value(dealerHand) < 17:  # Dealer must hit until their hand value is at least 17
            dealerHand.append(deck.pop())  # Deal an additional card to the dealer
            print(f"Dealer's hand: {' and a '.join(dealerHand)} (with a total value of {hand_value(dealerHand)})")

        if hand_value(dealerHand) > 21 or hand_value(playerHand) > hand_value(dealerHand):
